    ...
    internal sealed class WinGLContext : DesktopGraphicsContext
    {
            ...
            public TemporaryContext(INativeWindow native)
            {
                ...
                if (Context != ContextHandle.Zero)
                {
                    ...
                    for (int retry = 0; retry < 5 && !success; retry++)
                    {
                        ...
                        if (!success)
                        {
                            Console.WriteLine("wglMakeCurrent failed with error: {0}. Retrying", Marshal.GetLastWin32Error());
                ...
                {
                    Console.WriteLine("[WGL] CreateContext failed with error: {0}", Marshal.GetLastWin32Error());
                }
            ...
            int major, int minor, GraphicsContextFlags flags)
        {
            ...
            lock (LoadLock)
            {
                ...
                try
                {
                    ...
                    Console.WriteLine("OpenGL will be bound to window:{0} on thread:{1}", window.Handle,
                    ...
                    if (Wgl.SupportsFunction("wglCreateContextAttribsARB"))
                    {
                        try
                        {
                            ...
                            if (Handle == ContextHandle.Zero)
                                Console.WriteLine("failed. (Error: {0})", Marshal.GetLastWin32Error());
                        }
                    ...
                    if (Handle == ContextHandle.Zero)
                    {
                        ...
                        if (Handle == ContextHandle.Zero)
                            throw new GraphicsContextException(
                                string.Format(CultureInfo.InvariantCulture, "Context creation failed. Wgl.CreateContext() error: {0}.",
                                    Marshal.GetLastWin32Error()));
                    }

                    Debug.WriteLine(string.Format(CultureInfo.InvariantCulture, "success! (id: {0})", Handle));
                }
                finally
                {
            ...
            {
                Marshal.GetLastWin32Error();
                Debug.Write(string.Format(CultureInfo.InvariantCulture, "Sharing state with context {0}: ", sharedContext));
        ...
        public override void SwapBuffers()
        {
            if (!Functions.SwapBuffers(DeviceContext))
                throw new GraphicsContextException(string.Format(CultureInfo.InvariantCulture,
            ...
            set
            {
                lock (LoadLock)
                {
                    if (vsync_supported)
                    {
                        ...
                        {
                            Console.WriteLine("wglSwapIntervalEXT call failed.");
                        }
                    }
                }
            }
        ...
        */
        ...
        #endregion IWindowInfo IGLContextInternal.Info
        ...
        #endregion IGLContextInternal Members
        ...
        #endregion internal IntPtr DeviceContext
            ...
            if (Handle != ContextHandle.Zero)
            {
                try
                {
                    ...
                    if (!Wgl.DeleteContext(Handle.Handle))
                        Console.WriteLine("Failed to destroy OpenGL context {0}. Error: {1}",
                ...
                catch (AccessViolationException e)
                {
                    ...
                    Debug.Indent();
                    Console.WriteLine("Marshal.GetLastWin32Error(): {0}", Marshal.GetLastWin32Error().ToString(CultureInfo.InvariantCulture));
        ...
        #endregion private void DestroyContext()
