...
namespace Rann.Systems
{
    public static class TFunctions
    {
        ...
        //------------------------------------------------------------------------------------------------------------------------------------------------------
        //------------------------------------------------------------------------------------------------------------------------------------------------------
        public static double[,] FillDifferentialMatrix(TComponent[,] iDifferentialFormulasMatrix)
        {
            double[,] oDifferentialMatrix = new double[iDifferentialFormulasMatrix.GetUpperBound(0) + 1, iDifferentialFormulasMatrix.GetUpperBound(1) + 1 + iDifferentialFormulasMatrix.GetUpperBound(0) + 1];
            for (int i = 0; i <= iDifferentialFormulasMatrix.GetUpperBound(0); i++)
            {
                for (int j = 0; j <= iDifferentialFormulasMatrix.GetUpperBound(1); j++)
                {
                    oDifferentialMatrix[i, j] = iDifferentialFormulasMatrix[i, j].Position;
                    oDifferentialMatrix[i, iDifferentialFormulasMatrix.GetUpperBound(1) + i + 1] += iDifferentialFormulasMatrix[i, j].Position;
                }
                if (oDifferentialMatrix[i, iDifferentialFormulasMatrix.GetUpperBound(1) + i + 1] == 0)
                {
                    oDifferentialMatrix[i, iDifferentialFormulasMatrix.GetUpperBound(1) + i + 1] = 1;
                }

                oDifferentialMatrix[i, iDifferentialFormulasMatrix.GetUpperBound(1) + i + 1] /= (iDifferentialFormulasMatrix.GetUpperBound(0) + 1);
            }
